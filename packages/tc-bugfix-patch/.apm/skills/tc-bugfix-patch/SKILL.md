---
name: tc-bugfix-patch
description: 用于精准更新 spec.md、plan.md 和 tasks.md 以解决报告的 bug
---
...
4. If no bug report exists, inform the user and suggest running `/tc-bugfix-report` first
...
7. **Report**: Output a summary:
   ...
   - Suggest next step: `/tc-bugfix-verify` to confirm consistency, then `/tc.specify` to apply the code fix
...
- **Require bug report** — if no bug report or user description is provided, refuse to patch and suggest `/tc-bugfix-report` first
