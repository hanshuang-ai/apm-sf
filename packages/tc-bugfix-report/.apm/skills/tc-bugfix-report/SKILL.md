---
name: tc-bugfix-report
description: 用于捕获在实现过程中发现的 bug，并将其追溯到相关的规格、计划和任务产物
---
...
4. **Generate bug report**: Output a structured report:
   ...
   **Reported**: [DATE]
   ...
   ## Recommended Fix
   1. Run `/tc-bugfix-patch` to update spec artifacts
   2. Run `/tc-bugfix-verify` to confirm consistency
   3. Resume `/tc.specify` to apply the code fix
   ```
...
- **Never modify spec artifacts** — this command only reports, use `/tc-bugfix-patch` to make changes
