---
name: tc-brownfield-migrate
description: 用于为现有功能逆向工程Spec, 从而让功能可以被逐步采用SDD
---
...
4. Verify constitution exists (recommend running `/tc-brownfield-bootstrap` first if missing)
...
9. **Report**:
   ...
   ## Identified Gaps
   1. No error handling tests for expired tokens → consider `/tc.specify` for a follow-up feature
   2. No rate limiting on login endpoint → consider `/tc.bugfix.report` to track

   ## Next Steps
   - Review generated artifacts in `specs/user-auth/`
   - Use `/tc.refine.update` to adjust any inaccurate specs
   - Use `/tc.specify` for new features — they'll follow the same SDD workflow
   - Run `/tc-brownfield-migrate` again for additional features
   ```
