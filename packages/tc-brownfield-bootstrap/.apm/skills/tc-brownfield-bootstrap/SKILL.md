---
name: tc-brownfield-bootstrap
description: 用于生成针对现有代码库量身定制的 spec-kit 配置
---

# Bootstrap Spec-Kit

Generate a customized spec-kit configuration for an existing codebase. Uses the project profile from `/tc-brownfield-scan` (or performs a scan if none exists) to create a constitution, templates, and agent configuration that match the project's actual architecture, tech stack, and conventions.
...
1. **Load or generate project profile**: Check if `/tc-brownfield-scan` has been run:
...
9. **Report**:
   ...
   - Run `/tc-brownfield-validate` to verify configuration matches project
   - Run `/tc-brownfield-migrate` to reverse-engineer specs for existing features
   - Start new features with `/tc.specify` — templates are now project-aware
   ```
